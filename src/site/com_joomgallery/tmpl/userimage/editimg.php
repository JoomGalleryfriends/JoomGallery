                ...
                <?php if(false): ?>
                  <?php foreach($this->imagetypes as $key => $imagetype) : ?>
                    <a class="btn btn-outline-primary" style="cursor:pointer;"
                       data-imagetype="<?php echo $this->escape($imagetype->typename); ?>"
                       onclick="openModal(this.dataset.imagetype)"><?php echo $this->escape(Text::sprintf('COM_JOOMGALLERY_SHOWIMAGE_IMGTYPE', ucfirst($imagetype->typename))); ?></a>
                  <?php endforeach; ?>
                <?php endif ?>
              </div>
            </div>
