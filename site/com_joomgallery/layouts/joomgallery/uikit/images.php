...
$lightbox_attr   = $lightbox ? ' uk-lightbox="toggle: a[data-type];"' : '';
$grid_class      = 'uk-grid uk-child-width-1-1 uk-child-width-1-2@s uk-child-width-1-' . $columns . '@m uk-grid-' . $gap;
?>

<div id="lightgallery-<?php echo $id; ?>" class="jg-images jg-category <?php echo $grid_class; ?>"<?php echo $grid_attributes . $lightbox_attr; ?>>
  <?php foreach($items as $item) : ?>
    <?php
      $thumb = JoomHelper::getImg($item, $image_type);
      $large = JoomHelper::getImg($item, $lightbox_type);
      $href  = $lightbox ? $large : Route::_(JoomHelper::getViewRoute('image', (int) $item->id, (int) $item->catid));
    ?>
    <div class="jg-image">
      <div class="el-item">
        <a class="uk-display-block uk-width-1-1 uk-transition-toggle uk-inline-clip uk-link-toggle<?php echo $ratio ? ' uk-cover-container' : ''; ?>"
           href="<?php echo $href; ?>"
           <?php echo $lightbox ? 'data-type="image" role="button"' : ''; ?>>
          <?php if($ratio) : ?>
          ...
          <?php if($overlay != 'none') : ?>
            ...
            <div class="uk-position-center uk-transition-fade">
              <div class="uk-overlay uk-margin-remove-first-child uk-text-center">
                <?php if($title_position == 'overlay' && !empty($image_title)) : ?>
        ...
        <?php if($title_position == 'below' && !empty($image_title)) : ?>
