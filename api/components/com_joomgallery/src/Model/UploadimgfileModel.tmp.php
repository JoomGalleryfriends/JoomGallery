...
//
    ...
    public function save($path = null): string
    {
        ...
        $category_name = $this->getState('category_name', false);
...
//        $path = 'local-image:/' . 'joomgallery/' . $category_path . '/' . $image_name;
        ...
        $path          = '/joomgallery/originals/' . $category_name . '/' . $image_name;
        ...
        try
        {
            ...
            if($path && $content)
            {
                ...
                $name = $this->uploadApiModel->createFile($adapterName, $basename, $dirname, $content, $override);
        ...
        {
            throw new Save(Text::sprintf('WEBSERVICE_COM_MEDIA_FILE_NOT_FOUND', $dirname . '/' . $basename), 404);
        }
        catch (FileExistsException)
        {
            throw new Save(Text::sprintf('WEBSERVICE_COM_MEDIA_FILE_EXISTS', $dirname . '/' . $basename), 400);
        }
        catch (InvalidPathException)
        {
            throw new Save(Text::sprintf('WEBSERVICE_COM_MEDIA_BAD_FILE_TYPE', $dirname . '/' . $basename), 400);
        }

        // If we still have no result path, something fishy is going on.
        if(empty($resultPath))
        {
            throw new Save(Text::_('WEBSERVICE_COM_MEDIA_UNSUPPORTED_PARAMETER_COMBINATION'), 400);
        }
