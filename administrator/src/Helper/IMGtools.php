...
use \Joomgallery\Component\Joomgallery\Administrator\Helper\GifFrameExtractor;
use \Joomgallery\Component\Joomgallery\Administrator\Helper\GifCreator;
...
class IMGtools
{
                                      ...
                                      $anim = false, $unsharp = false)
  {
    ...
    $src_file = Path::clean($src_file);
    $dst_file = Path::clean($dst_file);

    // Load GifFrameExtractor-Class
    //JLoader::register('GifFrameExtractor', Path_COMPONENT_ADMINISTRATOR . '/helpers/gifframeextractor.php');

    // Analysis and validation of the source image
    if(!(self::$src_imginfo = self::analyseImage($src_file)))
    {
      $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_OUTPUT_INVALID_IMAGE_FILE').'<br />';
    ...
    {
      $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_GD_ONLY_JPG_PNG').'<br />';

      return false;
    }

    //$config = JoomConfig::getInstance();
    $config = Factory::getApplication()->bootComponent(_JOOM_OPTION)->config;
    ...
    if($noResize && self::$dst_imginfo['angle'] == 0 && self::$dst_imginfo['flip'] == 'none')
    {
      if($src_file != $dst_file)
      {
        ...
        $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_RESIZE_NOT_NECESSARY').'<br />';

        if(!File::copy($src_file, $dst_file))
        {
          $debugoutput .= Text::sprintf('COM_JOOMGALLERY_UPLOAD_OUTPUT_PROBLEM_COPYING', $dst_file).' '.Text::_('COM_JOOMGALLERY_COMMON_CHECK_PERMISSIONS').'<br />';
    ...
    if($src_file == $dst_file)
    {
      ...
      {
        $debugoutput .= Text::sprintf('COM_JOOMGALLERY_UPLOAD_OUTPUT_PROBLEM_COPYING', $src_file).' '.Text::_('COM_JOOMGALLERY_COMMON_CHECK_PERMISSIONS').'<br />';
    ...
    else
    {
      ...
      {
        if(!File::copy($dst_file, $dst_file.'bak'))
        {
          $debugoutput .= Text::sprintf('COM_JOOMGALLERY_UPLOAD_OUTPUT_PROBLEM_COPYING', $dst_file).' '.Text::_('COM_JOOMGALLERY_COMMON_CHECK_PERMISSIONS').'<br />';
    ...
    if(!$memory['success'])
    {
      $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_OUTPUT_ERROR_MEM_EXCEED').
    ...
    {
      case 1:
        $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_RESIZE_TO_HEIGHT');
        break;
      case 2:
        $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_RESIZE_TO_WIDTH');
        break;
      case 3:
        // Free resizing and cropping
        $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_RESIZE_TO_CROP');
        break;
      case 4:
        $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_RESIZE_TO_MAX');
        break;
      default:
        break;
    }
    ...
    switch($method)
    {
      ...
      case 'gd2':
        ...
        {
          $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_GD_LIBARY_NOT_INSTALLED').'<br />';
          self::rollback($src_file, $dst_file);
        ...
        if($anim && self::$src_imginfo['animation'] && self::$src_imginfo['type'] == 'GIF')
        {
          ...
          {
            $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_GD_LIBARY_NOT_ABLE_RESIZING').'<br />';
            self::rollback($src_file, $dst_file);
        ...
        if(self::$dst_imginfo['flip'] != 'none')
        {
          ...
          $debugoutput .= Text::sprintf('COM_JOOMGALLERY_FLIP_BY', self::$dst_imginfo['flip']).'<br />';
        }
        ...
        if(self::$dst_imginfo['angle'] > 0)
        {
          ...
          $debugoutput .= Text::sprintf('COM_JOOMGALLERY_ROTATE_BY_ANGLE', self::$dst_imginfo['angle']).'<br />';
        }
        ...
        if(!$noResize)
        {
          ...
          foreach(self::$src_frames as $key => $frame)
          {
            ...
            if($config->jg_fastgd2creation == 1)
            {
              $fast_resize = true;
            }
        ...
        {
          // Animated GIF image (image with more than one frame)
          //JLoader::register('GifCreator', Path_COMPONENT_ADMINISTRATOR . '/helpers/gifcreator.php');
          $gc = new GifCreator();
        ...
        if(!$success)
        {
          ...
          if($metadata)
          {
            ...
            if(!$meta_success)
            {
              $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_GD_ERROR_COPY_METADATA').'<br />';
              self::rollback($src_file, $dst_file);
      ...
      case 'im':
        ...
        foreach($disabled_functions as $disabled_function)
        {
          ...
          {
            $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_OUTPUT_EXEC_DISABLED').'<br />';
        ...
        else
        {
          ...
          {
            $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_OUTPUT_IM_NOTFOUND').'<br />';
        ...
        if($auto_orient)
        {
          ...
          $debugoutput .= Text::_('COM_JOOMGALLERY_AUTOORIENT_IMAGE').'<br />';
        }
        ...
        if($return_var != 0 || !$filecheck)
        {
          ...
          if($return_var != 0 || !$filecheck)
          {
            $debugoutput .= Text::sprintf('COM_JOOMGALLERY_UPLOAD_OUTPUT_IM_SERVERPROBLEM','exec('.$convert.');').'<br />';
            self::rollback($src_file, $dst_file);
        ...
        break;
      default:
        $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_UNSUPPORTED_METHOD').'<br />';
        self::rollback($src_file, $dst_file);
    ...
    {
      $debugoutput .= Text::sprintf('COM_JOOMGALLERY_COMMON_ERROR_RESIZE_IMAGE', $src_file).'<br />';
      self::rollback($src_file, $dst_file);
                                      ...
                                      $dst_qual = 100, $angle = 0, $auto_orient = true, $metadata = true, $anim = true)
  {
    ...
    {
      $debugoutput .= Text::_('COM_JOOMGALLERY_COMMON_ERROR_ROTATE_ONLY_JPG').'<br />';
    ...
    switch($method)
    {
      ...
      case 'gd2':
        if(!function_exists('imagecreate'))
        {
          $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_GD_LIBARY_NOT_INSTALLED');
          self::rollback($src_file, $dst_file);
        ...
        if(self::checkError(self::$src_frames) || self::checkError(self::$dst_frames))
        {
          $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_GD_LIBARY_NOT_ABLE_RESIZING');
          self::rollback($src_file, $dst_file);
        ...
        if(!$success)
        {
          ...
          if($metadata)
          {
            ...
            if(!$meta_success)
            {
              $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_GD_ERROR_COPY_METADATA');
              self::rollback($src_file, $dst_file);
      ...
      case 'im':
        ...
        else
        {
          if($angle > 0)
          {
            ...
            $debugoutput .= Text::sprintf('COM_JOOMGALLERY_ROTATE_BY_ANGLE', $angle).'<br />';
          }
        }
    ...
    //Factory::getApplication()->enqueueMessage($output);
    ...
    {
      $debugoutput .= Text::sprintf('COM_JOOMGALLERY_COMMON_ERROR_ROTATE_IMAGE', $src_file).'<br />';
      self::rollback($src_file, $dst_file);
                                         ...
                                         $wtm_pos, $wtm_resize, $wtm_newSize, $opacity = 0, $metadata = true, $anim = true)
  {
    ...
    {
      $debugoutput .= Text::_('COM_JOOMGALLERY_COMMON_ERROR_WATERMARK_NOT_EXIST');
    ...
    {
      $debugoutput .= Text::_('COM_JOOMGALLERY_COMMON_OUTPUT_INVALID_WTM_FILE').'<br />';
    ...
    // Create debugoutput
    $debugoutput .= Text::_('COM_JOOMGALLERY_COMMON_OUTPUT_WATERMARK_IMAGE');
    ...
    switch($method)
    {
      ...
      case 'gd2':
        ...
        if(self::checkError(self::$src_frames) || self::checkError(self::$dst_frames))
        {
          $debugoutput .= Text::_('COM_JOOMGALLERY_UPLOAD_GD_LIBARY_NOT_ABLE_WATERMARKING').'<br />';
          self::rollback($src_file, $dst_file);
    ...
    {
      $debugoutput .= Text::sprintf('COM_JOOMGALLERY_COMMON_ERROR_WATERMARKING_IMAGE', $src_file).'<br />';
      self::rollback($src_file, $dst_file);
  ...
  protected static function calculateMemory($src_imginfo, $dst_imginfo, $method)
  {
    ...
    switch($src_imginfo['type'])
    {
      ...
      case 'JPE':
        ...
        {
          case 'resize':
            if($config->jg_fastgd2creation && $gd2 && !$special)
            {
