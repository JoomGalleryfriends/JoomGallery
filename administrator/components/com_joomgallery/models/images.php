...
class JoomGalleryModelImages extends JoomGalleryModel
{
   ...
   * @param   string  $typestorotate  Parameter from JoomGallery configuration, 1=only thumbs and details, 2=thumbs, details and originals
   ...
   * @since   3.4
   */
  public function rotate($angle, $method = NULL, $quality = NULL, $typestorotate = NULL)
  {
      ...
      if($imginfo[2] == 'JPG')
      {
        ...
        switch($method)
        {
          ...
          case 'gd2':
            ...
            if((!$type || $type == 'orig') && $typestorotate == '2')
            {
              if($orig_existent)
              {
                // for debug:
                ...
                $rotated[$cid][] = 'orig';
                $debugoutput .= ' nach drehung original.';
              }
            }
            break;
          case 'im':
